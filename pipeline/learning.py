...
# Create regulation table function 
def create_regulation_table(consensus_predictions_f, features_f, out_f, regulatory_elements=None):
    if regulatory_elements is None:
        regulatory_elements = ['PmrA', 'CpxR', 'CsrA']

    predictions = pd.read_csv(consensus_predictions_f)
    features = pd.read_csv(features_f)
    id_col = predictions.columns[0]
    regulatory_cols = []
    regulatory_name_by_col = {}

    for element in regulatory_elements:
        matching_cols = [col for col in features.columns if col == element or col.startswith(f"{element}_")]
        for col in matching_cols:
            regulatory_cols.append(col)
            regulatory_name_by_col[col] = element

    if not regulatory_cols:
        empty_df = pd.DataFrame(columns=['OG', 'prediction score', 'present regulatory element'])
        empty_df.to_csv(out_f, index=False)
        return

    merged = predictions.merge(features[[id_col] + regulatory_cols], on=id_col, how='left')
    rows = []
    candidates = merged[merged['score'] > 0.5].sort_values(by='score', ascending=False)

    for _, row in candidates.iterrows():
        present_elements = []
        for col in regulatory_cols:
            if pd.notna(row[col]) and float(row[col]) == 1:
                present_elements.append(regulatory_name_by_col[col])
        if present_elements:
            rows.append([row[id_col], row['score'], ', '.join(present_elements)])
    regulation_df = pd.DataFrame(rows,columns=['OG', 'prediction score', 'present regulatory element'])
    regulation_df.head(10).to_csv(out_f, index=False)
...
# Create regulation table file
consensus_predictions_f = f'{out_dir}/consensus_predictions.csv'
if os.path.exists(consensus_predictions_f):
    create_regulation_table(consensus_predictions_f=consensus_predictions_f,features_f=filled_nan_f,out_f=f'{out_dir}/regulation.csv')
