...
from T4Es_wrapper import effectors_learn
import shutil
import re
import subprocess
import pandas as pd
...
def validate_gff(gff_f, ORFs_f, genome_name=''):
    ...
    with open(gff_f) as in_f:
        ...
        with open(gff_f, 'w') as out_f:
            out_f.write(content)
    CDS, RNA = regulatory_features.parse_gff(gff_f, locus_dic)
...
def validate_genome_and_gff(gff_f, genome_f, ORFs_f):
    ...
    locus_dic = fasta_parser.parse_ORFs(ORFs_f)
    regions = []
    locus_area_d, circulars = regulatory_features.parse_gff_to_CDS_loc(gff_f, locus_dic)
...
def validate_input(output_dir_path, ORFs_path, effectors_path, input_T4Es_path, host_proteome, genome_path, gff_path,
                   no_T4SS_path, error_path):
    ...
    if effectors_path:
        if effectors_path.endswith('.zip'):
            ...
            if error_msg:
                fail(error_msg, error_path)
    if input_T4Es_path:
        error_msg = verify_fasta_format(input_T4Es_path, 'protein', 'effectors for homology search')
        if error_msg:
            fail(error_msg, error_path)
        error_msg = validate_set(input_T4Es_path, 'Your effectors file for homology search')
        if error_msg:
            fail(error_msg, error_path)
        os.makedirs(f'{output_dir_path}/blast_data', exist_ok=True)
        cmd = f'cp {blast_datasets_dir}/*.faa {output_dir_path}/blast_data/'
        subprocess.check_output(cmd, shell=True)
        eff1_recs = SeqIO.parse(f'{output_dir_path}/blast_data/T4Es.faa', 'fasta')
        eff_l = list(eff1_recs)
        seqs = [rec.seq for rec in eff_l]
        eff2_recs = SeqIO.parse(input_T4Es_path, 'fasta')
        for rec in eff2_recs:
            if rec.seq not in seqs:
                eff_l.append(rec)
                seqs.append(rec.seq)
        SeqIO.write(eff_l, f'{output_dir_path}/blast_data/T4Es.faa', 'fasta')

        if os.path.exists(f'{output_dir_path}/Effectidor_runs'):
            for genome in os.listdir(f'{output_dir_path}/Effectidor_runs'):
                shutil.copy(input_T4Es_path, f'{output_dir_path}/Effectidor_runs/{genome}')
    if host_proteome:
    ...
    if no_T4SS_path:
        logger.info(f'calling verify_zip {no_T4SS_path}')
        error_msg = verify_zip(no_T4SS_path, 'Close bacteria without T4SS data')
        if error_msg:
            fail(error_msg, error_path)
        os.makedirs(f'{output_dir_path}/blast_data/temp_extract', exist_ok=True)
        shutil.unpack_archive(no_T4SS_path, f'{output_dir_path}/blast_data/temp_extract')
...
def main(ORFs_path, output_dir_path, effectors_path, input_T4Es_path, host_proteome, html_path, queue, genome_path,
         gff_path, no_T4SS, identity_cutoff='50', coverage_cutoff='60', PmrA=False, CpxR=False, CsrA=False, exs=False,
    ...
    try:
        ...
        validate_input(output_dir_path, ORFs_path, effectors_path, input_T4Es_path, host_proteome, genome_path,
                       gff_path, no_T4SS, error_path)
        ...
        if os.path.exists(f'{output_dir_path}/Effectidor_runs'):
            ...
            for genome in os.listdir(f'{output_dir_path}/Effectidor_runs'):
                ...
                job_cmd = f'module load MMseqs2/May2024;!@#python {os.path.join(scripts_dir, "T4Es_wrapper.py")} ' \
            ...
            effectors_learn(error_path, ORFs_path, effectors_path, output_dir_path, tmp_dir, queue, gff_path,
                            genome_path, PmrA=PmrA, CpxR=CpxR, CsrA=CsrA, exs=exs, tts=tts,
        ...
        if positives_size == 0:
            error_msg = 'No effectors were found in your data based on homology! If you know your data should contain '\
                        'type IVb effectors please supply them. '
            fail(error_msg, error_path)
        ...
        T4SS_data = pd.read_csv(os.path.join(output_dir_path, 'T4SS.csv'), dtype=str)
        T4SS_table = T4SS_data.dropna()
        T4SS_table = T4SS_table.to_html(index=False, justify='left', escape=False)
        
        # Add regulation table:
        regulation_table = ''
        regulation_f = os.path.join(output_dir_path, 'out_learning', 'regulation.csv')
        if os.path.exists(regulation_f):
            regulation_data = pd.read_csv(regulation_f, dtype=str)
            if not regulation_data.empty:
                annotations_df = pd.read_csv(f'{output_dir_path}/OGs_annotations.csv', dtype=str)

                # Add annotation column to regulation table
                regulation_data = regulation_data.merge(
                    annotations_df[['OG', 'Annotation(s)']],
                    on='OG',
                    how='left'
                )
                cols = list(regulation_data.columns)
                if 'Annotation(s)' in cols:
                    cols.remove('Annotation(s)')
                    if 'prediction_score' in cols:
                        insert_after = cols.index('prediction_score') + 1
                    elif 'Prediction score' in cols:
                        insert_after = cols.index('Prediction score') + 1
                    else:
                        insert_after = 2
                    cols.insert(insert_after, 'Annotation(s)')
                    regulation_data = regulation_data[cols]
                regulation_table = regulation_data.to_html(index=False, justify='left', escape=False)
            
        if html_path:
            # shutil.make_archive(final_zip_path, 'zip', output_dir_path)
            finalize_html(html_path, error_path, run_number, predicted_table, positives_table, T4SS_table, regulation_table, low_quality_flag, output_dir_path)
        else:
            with open(f'{output_dir_path}/output.html', 'w') as out:
                out.write(f'positives:\n{positives_table}\n<br>\npredicted:\n{predicted_table}<br>'f'T4SS components:<br>{T4SS_table}<br>Regulation:<br>{regulation_table}')
...
def finalize_html(html_path, error_path, run_number, predicted_table, positives_table, T4SS_table, regulation_table, low_quality_flag, output_dir_path):
    ...
    if succeeded:
        ...
        edit_success_html(CONSTS, html_path, predicted_table, positives_table, T4SS_table, regulation_table, low_quality_flag, output_dir_path)
    else:
...
def edit_success_html(CONSTS, html_path, predicted_table, positives_table, T4SS_table, regulation_table, low_quality_flag, output_dir_path):
    ...
    if predicted_table:
        append_to_html(html_path, f'''
                ...
                <br>
                <a href='T4SS.csv' target='_blank'>Download T4SS components' details</a>
                <br>
                ...
                {predicted_table}
                {f"<br><h3><b>Effectors Regulation – Top 10 predictions with detected regulatory elements</b></h3><br>{regulation_table}" if regulation_table else ""}
                </div>
        ...
        if os.path.exists(f'{output_dir_path}/Effectidor_runs'):
            ...
            if num_genomes > 1:
                append_to_html(html_path, f'''
                    <br>
                    <h3><b>T4Es presence/absence map</b></h3>
                    <br>
                    <a href='T4Es_presence_absence.png'><img src='T4Es_presence_absence.png' style="max-width: 100%;" /></a>
                    <br>
                    <h3><b>T4SS components presence/absence map</b></h3>
                    <a href='T4SS_presence_absence.png'><img src='T4SS_presence_absence.png' style="max-width: 100%;" /></a>
                    ''')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_T4Es_path', default='',
                        help='A path to protein fasta with T4Es records of other bacteria.')
    parser.add_argument('--host_proteome_path', default='',
                        help='A path to a zip archive with protein fasta files of host proteome.')
    parser.add_argument('--no_T4SS', default='',
                        help='A path to a zip archive with protein fasta files with related bacteria non T4SS '
                             'proteomes.')
    ...
    main(args.input_ORFs_path, args.output_dir_path, args.input_effectors_path, args.input_T4Es_path,
         args.host_proteome_path, args.html_path, args.queue_name, args.genome_path, args.gff_path, args.no_T4SS,
         identity_cutoff=args.identity_cutoff, coverage_cutoff=args.coverage_cutoff, PmrA=PmrA_flag, CpxR=CpxR_flag,
         CsrA=CsrA_flag, exs=exs_flag, tts=tts_flag, homology_search=args.homology_search,
