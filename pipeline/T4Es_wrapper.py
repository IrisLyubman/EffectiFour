...
def write_sh_file(tmp_dir, path_to_fasta, path_for_output_csv, queue='pupko-pool'):
    cmds = [
        'export HOME=/groups/pupko/irislyubman/secretion_signal_type4b/',
        ...
        'conda activate /groups/pupko/irislyubman/.conda/envs/secretion_signal',
        'export PATH=$CONDA_PREFIX/bin:$PATH',
        'which python',
        'echo "CONDA_PREFIX=$CONDA_PREFIX"',
        f'python ~/T4Es_secretion_signal_prediction/src/inference/predict_secretion_signal.py '
        f'--input_fasta_file {path_to_fasta} '
        f'--output_file {path_for_output_csv} --load_llm_from_disk',
        f'touch {path_for_output_csv}.done',
        f'touch {os.path.join(tmp_dir, "Embedding_pred.csv.done")}'
    ]

    content = '!@#'.join(cmds) + '\tEffectidor_embedding'
...
def effectors_learn(error_path, ORFs_file, effectors_file, working_directory, tmp_dir, queue, gff_file, full_genome_f,
                    PmrA=False, CpxR=False, CsrA=False, exs=False, tts=False, homology_search=False, signal=False,
    ...
    if not os.path.exists(f'{working_directory}/blast_data/T4Es.faa'): #if no input for effectors homology search was supplied
        os.makedirs(f'{working_directory}/blast_data',exist_ok=True) # this directory was already created if another input for homology search was supplied (host proteome or proteomes of closely related bacteria without T4SS)
        cmd = f'cp {blast_datasets_dir}/*.faa {working_directory}/blast_data/'
        subprocess.check_output(cmd,shell=True)
    cmd = f'cp -r {os.path.join(data_dir,"T4SS_data")} {working_directory}'
    subprocess.check_output(cmd, shell=True)
    ...
    if signal: #make C-terminal sequences for Signal search
        ...
        for rec in recs:
            ...
            rec.seq = C_seq
            rec.description = ID
            C_terminals.append(rec)
        C_terminal_file_path = os.path.join(working_directory, 'C_terminals.faa')
        SeqIO.write(C_terminals, C_terminal_file_path, 'fasta')
        write_sh_file(tmp_dir, C_terminal_file_path, f'{working_directory}/Embedding_pred.csv')
    
    if not effectors_file:
        subprocess.check_output(['python', f'{scripts_dir}/find_effectors.py', f'{blast_datasets_dir}/T4Es.faa', all_prots, effectors_prots, coverage])
    elif homology_search:
        effectors_prots2 = 'homology_found_effectors.faa'
        subprocess.check_output(['python', f'{scripts_dir}/find_effectors.py', f'{blast_datasets_dir}/T4Es.faa', all_prots, effectors_prots2, coverage])
    ...
    T4Es_recs = SeqIO.parse(effectors_prots,'fasta')
    if len(list(T4Es_recs))>400:
        error_msg = 'the number of effectors found based on homology exceeds the number of expected T4Es in a single genome! If you included T4Es for homology search, make sure they contain nothing but true T4Es.'
    ...
    subprocess.check_output(['python', f'{scripts_dir}/find_T4SS_components.py', working_directory, all_prots, 'T4SS_data'])
    ...
    with open(f'{working_directory}/features_jobs.cmds','w') as jobs_f:
        ...
        if gff_file:
            ...
            if full_genome_f:
                ...
                cmds = f'{CONSTS.MODULE_LOAD}; python {scripts_dir}/regulatory_features.py {ORFs_file} {working_directory} {gff_file} {full_genome_f}'
                if PmrA:
                    cmds += ' --PmrA'
                if CpxR:
                    cmds += ' --CpxR'
                ...
                if tts:
                    cmds += ' --tts'
                cmds += f'!@#touch {os.path.join(tmp_dir,"regulatory_features.done")}\tcis_regulatory_elements\n'
                jobs_f.write(cmds)
        # signal peptide
    if signalp:
    ...
    if gff_file:
        ...
        if full_genome_f:
            files_to_merge.append('regulatory_features.csv')
            done_files.append('regulatory_features.done')
...
if __name__ == '__main__':
        ...
        effectors_learn(args.error_path, args.input_ORFs_path, args.input_effectors_path, args.output_dir_path,
                        f'{args.output_dir_path}/tmp', args.queue, args.gff_file, args.full_genome_f, PmrA=args.PmrA,
                        CpxR=args.CpxR, CsrA=args.CsrA, exs=args.exs, tts=args.tts, homology_search=args.homology_search,
