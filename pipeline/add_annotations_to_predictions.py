...
def add_annotations_to_predictions(in_f, out_f_normal, out_f_pseudo, annotations_fasta,
    ...
    T4SS_recs = SeqIO.to_dict(SeqIO.parse('T4SS_proteins.faa', 'fasta'))
    T4_hits_f = 'T4SS_hits.csv'
    T4_d = {}
    with open(T4_hits_f) as f:
        reader = csv.reader(f)
        for row in reader:
            T4_d[row[0]] = row[1].replace('\t', ', ')
    locus_prot = {}
    for rec in recs:
        dna_seq = rec.seq
    ...
    with open(in_f) as f:
        ...
        with open(out_f_normal, 'w', newline='') as out_normal:
            ...
            with open(out_f_pseudo, 'w', newline='') as out_pseudo:
                ...
                with open(out_f_T4SS, 'w', newline='') as out_T4SS:
                    writer_T4SS = csv.writer(out_T4SS)
                    header = next(reader)
                    header.append('Annotation')
                    header.append('Protein sequence')
                    writer_normal.writerow(header)
                    writer_pseudo.writerow(header)
                    writer_T4SS.writerow(['Locus tag']+header[-2:]+['Hits to T4SS proteins (see Data)'])
                    for row in reader:
                        row.append(locus_annotation[row[0]])
                        row.append(locus_prot[row[0]])
                        if row[0] in T4SS_recs:
                            writer_T4SS.writerow([row[0]]+row[-2:]+[T4_d[row[0]]])
...
def make_html_tables(predictions_f, effector_homolog, T4SS=''):
    ...
    positives_table = positives.merge(effector_homolog_df, how='left').to_html(
        index=False, justify='left', escape=False)
    # T4SS_data = pd.read_csv(T4SS)
    # T4SS_table = T4SS_data.to_html(index=False,justify='left',escape=False)
