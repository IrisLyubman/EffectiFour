...
sys.path.append('/effectidor/type4b/script/')
...
from submit_slurm_new import submit_job_to_Q  # auxiliaries
...
def write_html_prefix(output_path, run_number):
    ...
    <link rel="shortcut icon" type="image/x-icon" href="{CONSTS.WEBSERVER_URL}/type4b/pics/new_logo.gif" />
   ...
   <link rel="stylesheet" href="/type4b/css/general.css?v=999">
        ...
        <div class="row" id="jumbo">
            ...
            <div class="col-md-10" align="center">
                 <img src="/type4b/pics/new_logo.gif?v=2" id="nav_bar_image">
            </div>
    </div>
        ...
        <div class="container-fluid" id="nav_container">
                    <div style="display:table; margin:0 auto;">
                        <ul class="nav navbar-nav"  id="menu-nav">
                            <li class="nav-item" style="padding: 0px 20px;"><a href="/type4b/index.html">Home</a></li>
                            <li class="nav-item" style="padding: 0px 20px;"><a href="/tutorial.html">Tutorials</a></li>
                            <li class="nav-item" style="padding: 0px 20px;"><a href="/type4b/FAQ.html">FAQ</a></li>
                            <li class="nav-item" style="padding: 0px 20px;"><a href="/type4b/data.html">Data</a></li>
                            <li class="nav-item" style="padding: 0px 20px;"><a href="/type4b/credits.html">Credits</a></li>
...
def write_running_parameters_to_html(cgi_debug_path, output_path, job_title, ORFs_name, effectors_name, T4Es_name,
                                     host_name, non_T4SS_name, gff_name, genome_f_name, PmrA, CpxR, CsrA, exs, tts,
                                     T4signal, sigp):
    with open(output_path, 'a') as f:
        ...
        if T4Es_name != '':
            ...
            f.write('<div class="row" style="font-size: 20px;">')
            f.write('<div class="col-md-12">')
            f.write(f'<b>T4Es of other bacteria input file: </b>{T4Es_name}')
            f.write('</div></div>')
        ...
        if non_T4SS_name != '':
            ...
            f.write('<div class="row" style="font-size: 20px;">')
            f.write('<div class="col-md-12">')
            f.write(f'<b>bacterial proteomes without T4SS archive: </b>{non_T4SS_name}')
            f.write('</div></div>')
        ...
        if T4signal:
            ...
            f.write('<div class="row" style="font-size: 20px;">')
            f.write('<div class="col-md-12">')
            f.write(f'<b>Including Type IVb secretion signal prediction</b>')
            f.write('</div></div>')
...
def run_cgi():
    ...
    wd = os.path.join(CONSTS.EFFECTIDOR_RESULTS_DIR, run_number)
    create_dir(wd)
    output_path = os.path.join(wd, 'output.html')
    cgi_debug_path = os.path.join(wd, 'cgi_debug.txt')
    
    #IRIS DEBUG
    write_to_debug_file(cgi_debug_path, f'CGI script path: {os.path.abspath(__file__)}')
    write_to_debug_file(cgi_debug_path, f'CGI script mtime: {ctime(os.path.getmtime(__file__))}')
    ...
    try:
        ...
        if len(form['ORFs'].value) < 100:
            write_to_debug_file(cgi_debug_path, 'ORFs file is too small')
        ...
        with open(cgi_debug_path, 'a') as f:
            ...
            for key in sorted_form_keys:
                if 'ORFs' in key or 'effectors' in key or 'host' in key or 'no_T4SS' in key or 'gff' in key or 'genome' in key or 'T4Es' in key:
        ...
        if form['T4Es'].value:  # not empty string / empy bytes - the file was supplied by the user
            write_to_debug_file(cgi_debug_path, f'12\n')
            T4Es_path = os.path.join(wd, 'user_T4Es.fasta')
            upload_file(form, 'T4Es', T4Es_path, cgi_debug_path)
            write_to_debug_file(cgi_debug_path, f'user_T4Es file was saved to disk successfully\n\n')

            parameters += f' --input_T4Es_path {T4Es_path}'
        ...
        if form['no_T4SS'].value:  # not empty string / empy bytes - the file was supplied by the user
            write_to_debug_file(cgi_debug_path, f'14\n')
            no_T4SS_path = os.path.join(wd, 'non_T4SS.zip')
            upload_file(form, 'no_T4SS', no_T4SS_path, cgi_debug_path)
            write_to_debug_file(cgi_debug_path, f'no_T4SS file was saved to disk successfully\n\n')

            parameters += f' --no_T4SS {no_T4SS_path}'

        parameters += f' --identity_cutoff {identity}'
        parameters += f' --coverage_cutoff {coverage}'
        PmrA, CpxR, CsrA, exs, tts = False, False, False, False, False
        ...
        if form['genome'].value:  # not empty string / empy bytes - the file was supplied by the user
            ...
            if 'tts-box' in form:
                ...
                parameters += ' --tts'
                tts = True
        sigp, T4signal = False, False
        if translocation_signal == 'yes':
            T4signal = True
        # if signalp == 'yes':
        #    sigp = True
        write_to_debug_file(cgi_debug_path, f'23\n')
        write_running_parameters_to_html(cgi_debug_path, output_path, job_title, ORFs_name, effectors_name, T4Es_name,
                                         host_name, non_T4SS_name, gff_name, genome_f_name, PmrA, CpxR, CsrA, exs, tts,
                                         T4signal, sigp)
        ...
        create_slurm_file_cmd = " ".join(
            ["python", f"{CONSTS.Q_SUBMIT_SCRIPT}", cmds_file, wd, f'-q {CONSTS.QUEUE_NAME}', '--no_run true'])
        ...
        if user_email != '':
            ...
            if T4Es_name:
                notification_content += f'type IVb effectors of other bacteria file: {T4Es_name}\n'
            if host_name:
                notification_content += f'host proteome file: {host_name}\n'
            if non_T4SS_name:
                notification_content += f'proteomes with no T4SS archive: {non_T4SS_name}\n'
            if gff_name:
        ...
        html_content += f'<br><br><br><center><h2><font color="red">{msg}</font><br><br>Please make sure all files are in the right format and try to re-run your job or <a href="mailto:{CONSTS.ADMIN_EMAIL}?subject={CONSTS.PmrAELINE_NAME}%20Run%20Number%20{run_number}">contact us</a> for further information</h2></center><br><br>\n</body>\n</html>\n'
        with open(output_path, 'w') as f:
